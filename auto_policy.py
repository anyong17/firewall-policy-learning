...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2 or sys.argv[1] == "-h":
        usage = "Usage:" + sys.argv[0] + " input.csv(输入数据集) {output.csv(输出数据集)} {学习深度(1|2)}".decode('utf-8').encode(type)
        print usage
        sys.exit()
    ...
    print "分析开始...".decode('utf-8').encode(type)
    dtf_count = dtf.iloc[:,0].size
    print "数据集总数为：".decode('utf-8').encode(type), dtf_count
    print "数据集总共包含不同的目的地址数为：".decode('utf-8').encode(type), dtf.dip.drop_duplicates().count()
    print "数据集总共包含不同的目的端口数为：".decode('utf-8').encode(type), dtf.dport.drop_duplicates().count()
    print "数据集总共包含不同的源地址数为：".decode('utf-8').encode(type), dtf.sip.drop_duplicates().count()
    print "数据集总共包含不同的协议数为：".decode('utf-8').encode(type), dtf.proto.drop_duplicates().count()
    #dip_dtf = get_dip_dtf(dtf)
    tcp_dtf = dtf[dtf['proto'] == 6]
    udp_dtf = dtf[dtf['proto'] == 17]
    other_dtf = dtf[(dtf['proto'] != 6) & (dtf['proto'] != 17)]
    print "TCP协议分析开始...".decode('utf-8').encode(type),tcp_dtf.iloc[:,0].size
    study_loop(tcp_dtf,6)
    print "UDP协议分析开始...".decode('utf-8').encode(type),udp_dtf.iloc[:,0].size
    study_loop(udp_dtf,17)
    print "其他协议分析开始...".decode('utf-8').encode(type),other_dtf.iloc[:,0].size
    study_not_tcpudp_loop(other_dtf)
    print "分析结束...".decode('utf-8').encode(type)
    print "学习出元策略个数：".decode('utf-8').encode(type),g_output_df.dip.count()
    print "结果排序...".decode('utf-8').encode(type)
    ...
    if (deep_level == 1):
        ...
        print "深度2分析开始...".decode('utf-8').encode(type)
        second_study_dtf = second_study(g_output_df)
        print "深度2分析结束...".decode('utf-8').encode(type),second_study_dtf.dip.count()
