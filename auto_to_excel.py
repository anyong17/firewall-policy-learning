...
def first_to_excel(dtf):
    ...
    for i in dtf.index:
        # print dtf[i].dip
        dip = dtf.loc[i].dip.astype(numpy.int)
        dip = ctypes.c_uint32(dip).value
        if (BYTE_ORDER == 0):
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2 or sys.argv[1] == "-h":
        usage = "Usage:" + sys.argv[0] + " file.csv(输入数据集) {summary.xlsx(输出excel)} {学习深度(1|2)} {host|network} ".decode('utf-8').encode(type)
        print usage
        sys.exit()
    ...
    if (deep_level == 1):
        ...
        second_to_excel(dtf)
    print "转换为Excel文件结束...".decode('utf-8').encode(type)